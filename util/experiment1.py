...
def simulate_dataset(train_example_count: int, test_example_count, data_path: Path, confounder_p_train, confounder_p_test: float,
                     immune_state_signals: list, experiment_name: str, immune_state_p_conf1, immune_state_p_conf2, confounder_signal,
    ...
    train_graph = make_graph(confounder_p=confounder_p_train, immune_state_p_conf1=immune_state_p_conf1,
                             ...
                             immune_state_signals=immune_state_signals, confounder_signal=confounder_signal, experiment_name=experiment_name,
    ...
    train_dataset = make_dataset(repertoire_paths=study_cohort_data["repertoire"], path=data_path / 'train',
                                 dataset_name=f"experiment{experiment_name}_train", signal_names=[immune_state_signals[0].id, 'confounder'])

    # # # make a test dataset

    test_graph = make_graph(confounder_p_test, immune_state_p_conf1, immune_state_p_conf2, data_path, sequence_count, immune_state_signals,
    ...
    test_dataset = make_dataset(repertoire_paths=test_data["repertoire"], path=data_path / 'test',
                                dataset_name=f"experiment{experiment_name}_test", signal_names=[immune_state_signals[0].id, 'confounder'])
...
def make_repertoire_node(immune_state_node, confounder_node, data_path, train, immune_state_signals, sequence_count, confounder_signal,
    ...
    return Node(name="repertoire", function=get_repertoire,
                kwargs={"immune_state": immune_state_node, "confounder": confounder_node,
                        ...
                        "sequence_count": sequence_count, "immune_state_signals": immune_state_signals,
...
def make_graph(confounder_p: float, immune_state_p_conf1: float, immune_state_p_conf2: float, data_path: Path, sequence_count: int,
    ...
    repertoire_node = make_repertoire_node(immune_state_node, confounder_node, data_path, train, immune_state_signals, sequence_count,
